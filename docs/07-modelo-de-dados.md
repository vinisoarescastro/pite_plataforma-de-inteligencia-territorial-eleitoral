...
### `bairro`
Criada pela migration `l9a0b1c2d3e4`. Gerenciada por `backend/models/eleitoral.py` e `backend/geo/router.py`.

| Campo | Tipo | Descrição |
|---|---|---|
| `id` | UUID (PK) | Identificador único (`gen_random_uuid()`) |
| `nm_bairro` | String(100) | Nome do bairro |
| `sg_uf` | String(2) | Sigla da UF |
| `cd_municipio_ibge` | String(7) (nullable) | Código IBGE do município |
| `nm_municipio` | String(100) (nullable) | Nome do município (desnormalizado) |
| `geom` | GEOMETRY(GEOMETRY, 4326) (nullable) | Polígono desenhado pelo usuário via Leaflet-Geoman |
| `criado_em` | DateTime | Data de criação |

Índices: `ix_bairro_municipio (sg_uf, cd_municipio_ibge)`, `ix_bairro_geom (GIST)`.

### `bairro_local_votacao`
Criada pela migration `l9a0b1c2d3e4`. Vínculo manual entre bairro e local de votação.

| Campo | Tipo | Descrição |
|---|---|---|
| `bairro_id` | UUID (FK → bairro) | Bairro vinculado |
| `sg_uf` | String(2) | UF do local |
| `cd_municipio_tse` | String(10) | Código TSE do município |
| `nr_local_votacao` | Integer | Número do local de votação (`NR_LOCAL_VOTACAO` do TSE) |
| `nm_local_votacao` | String(200) (nullable) | Nome do local (cache para exibição) |
| `ds_endereco` | String(500) (nullable) | Endereço (cache para exibição) |

PK composta: `(bairro_id, sg_uf, cd_municipio_tse, nr_local_votacao)`.

### `local_votacao_geo`
Criada pela migration `n1c2d3e4f5g6`. Armazena coordenadas geocodificadas de locais de votação via Nominatim.

| Campo | Tipo | Descrição |
|---|---|---|
| `sg_uf` | String(2) | UF do local |
| `cd_municipio_tse` | String(10) | Código TSE do município |
| `nr_local_votacao` | Integer | Número do local de votação |
| `nm_local_votacao` | String(200) (nullable) | Nome do local |
| `ds_endereco` | String(500) (nullable) | Endereço do local |
| `lat` | Double Precision (nullable) | Latitude geocodificada |
| `lon` | Double Precision (nullable) | Longitude geocodificada |
| `geom` | GEOMETRY(POINT, 4326) (nullable) | Ponto geográfico (`ST_MakePoint(lon, lat)`) |
| `status` | String(10) | `pendente`, `ok`, `erro` |
| `geocodificado_em` | TimestampTZ (nullable) | Data/hora da geocodificação |

PK composta: `(sg_uf, cd_municipio_tse, nr_local_votacao)`.  
Índices: `ix_local_votacao_geo_mun (sg_uf, cd_municipio_tse)`, `ix_local_votacao_geo_geom (GIST)`, `ix_local_votacao_geo_status`.

**Fluxo de geocodificação:**
1. `POST /geo/geocoding/municipio` registra os locais pendentes e dispara `BackgroundTask`.
2. `geocoding.py` chama Nominatim (1 req/s), tenta endereço e fallback para nome.
3. `GET /geo/geocoding/status` expõe contagens por status; frontend faz polling a cada 3 s.
4. `POST /geo/bairros/{id}/sugerir-locais` usa `ST_Within(geom, polígono)` para retornar locais dentro do polígono desenhado.
...
> **Status: implementado.** Tabela `bairro` ativa no banco. Ver também seção [Tabelas Implementadas](#tabelas-implementadas-mvp--estado-atual) acima.

**Objetivo:** Granularidade territorial abaixo do município. Permite análise de votos por região urbana. O polígono de cada bairro é **desenhado manualmente** pelo administrador via interface Leaflet-Geoman na página de Geografia.
...
| `cd_municipio_ibge` | String(7) (nullable) | Código IBGE do município |
| `nm_municipio` | String(100) (nullable) | Nome do município (desnormalizado) |
| `geom` | GEOMETRY(GEOMETRY, 4326) (nullable) | Polígono desenhado via Leaflet-Geoman |
| `criado_em` | DateTime | Data de criação |

**Vínculo com locais de votação:** feito via tabela `bairro_local_votacao` — pode ser vinculado manualmente (lista de locais no painel lateral) ou automaticamente ao desenhar um polígono (sugestão via `ST_Within` nos locais geocodificados de `local_votacao_geo`).

```sql
-- Sugestão automática de locais dentro do polígono desenhado
SELECT g.*
FROM local_votacao_geo g
WHERE g.status = 'ok'
  AND ST_Within(g.geom, ST_SetSRID(ST_GeomFromGeoJSON(:poligono), 4326))
  AND NOT EXISTS (
      SELECT 1 FROM bairro_local_votacao blv
      WHERE blv.bairro_id = :bairro_id
